...
function EnviosClientes() {
  ...
  const sumarizarPedidosPorCliente = () => {
    ...
    pedidos.forEach((pedido) => {
      const { nombre_cliente, total, cantidad, fecha_entrega, localidad } =
        pedido;
    ...
    { field: "estado_pago", headerName: "Estado del Pago", flex: 1 },
    {
      ...
      renderCell: (params) => (
        <Box>
          <Button
            color="success"
            onClick={() => handleDetalle(params.row.cliente)}
            size="small"
          ...
          <CopyToClipboard
            text={mensaje}
            onCopy={() => handleCopyToClipboard(params.row.cliente)}
          >
  ...
  return (
    <Box>
      ...
      <Box style={{ margin: "1rem 0" }} align="center">
        {copied && (
          <Alert severity="success">Mensaje copiado al portapapeles.</Alert>
        )}
      </Box>
    </Box>
  );
