...
function ActualizaMasivaPedidos() {
    ...
    const fetchPedidos = async () => {
      try {
        ...
        const uniqueClientes = Array.from(
          new Set(data.map((pedido) => pedido.nombre_cliente))
        );
  ...
  const handleGuardar = () => {
    ...
    const pedidosClientes = pedidos.filter((pedido) =>
      selectedClientes.includes(pedido.nombre_cliente)
    );
    ...
    const pedidosActualizados = pedidosClientesFecha.map((pedido) => ({
      ...pedido,
      fecha_entrega: checkFechaEntrega
        ? selectedFechaEntrega
          ? new Date(selectedFechaEntrega)
          : ""
        : pedido.fecha_entrega,
      localidad: checkLocalidad ? selectedLocalidad : pedido.localidad,
      estado_pedido: checkEstadoPedido
    ...
    pedidosActualizados.forEach((pedido) => {
      axios
        .put(`https://vivosis.vercel.app/api/pedido/${pedido._id}`, pedido)
        .then((response) => {
        ...
        <Card sx={{ display: "flex" }}>
          <Box sx={{ display: "flex", flexDirection: "column" }}>
            <CardContent sx={{ flex: "1 0 auto" }}>
              ...
              >
                Actualizar pedidos (pendientes) de clientes
              </Typography>
                ...
                <Box width={800}>
                  ...
                  <div>
                    <FormControlLabel
                      ...
                      control={
                        <Switch
                          checked={checkFechaEntrega}
                          onChange={handleCheckFechaEntregaChange}
                        />
                      }
                      label="Actualizar"
                    />
                  ...
                  <div>
                    <FormControlLabel
                      ...
                      control={
                        <Switch
                          checked={checkLocalidad}
                          onChange={handleCheckLocalidadChange}
                        />
                      }
                      label="Localidad"
                    />
                  ...
                  <div>
                    <FormControlLabel
                      ...
                      control={
                        <Switch
                          checked={checkEstadoPedido}
                          onChange={handleCheckEstadoPedidoChange}
                        />
                      }
                  ...
                  <div>
                    <FormControlLabel
                      ...
                      control={
                        <Switch
                          checked={checkEstadoPago}
                          onChange={handleCheckEstadoPagoChange}
                        />
                      }
