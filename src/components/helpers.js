...
export const actualizarStock = async (idArticulo, cantidad) => {
  try {
    ...
    const response = await fetch(
      `https://vivosis.vercel.app/api/articulo/${idArticulo}`,
      {
        method: "PUT",
        headers: {
...
export const guardarCambiosEnProducto = async (
  idPedido,
  idArticulo,
  cantidad
) => {
  try {
    ...
    if (stockActualizado) {
      ...
      const response = await fetch(
        `https://vivosis.vercel.app/api/pedido/${idPedido}`,
        {
          method: "DELETE",
        }
      );
      ...
      console.log(
        "Error al actualizar el stock del artículo, no se elimina el pedido"
      );
      return false;
    }
  } catch (error) {
