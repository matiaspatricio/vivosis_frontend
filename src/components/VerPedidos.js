...
function VerPedidos() {
  ...
  const [filterNombreArticuloOptions, setFilterNombreArticuloOptions] =
    useState([]);
  ...
  useEffect(() => {
    const nombreClienteOptions = Array.from(
      new Set(pedidos.map((pedido) => pedido.nombre_cliente))
    );
    setFilterNombreClienteOptions(nombreClienteOptions);
  }, [pedidos]);

  useEffect(() => {
    const nombreArticuloOptions = Array.from(
      new Set(pedidos.map((pedido) => pedido.nombre_articulo))
    );
  ...
  const confirmActMasiva = async () => {
    ...
    const selectedPedidos = pedidos.filter((pedido) =>
      selectedRows.includes(pedido.id)
    );
    ...
    if (dialogAction === "modificarLocalidad") {
      ...
      for (const pedido of selectedPedidos) {
        pedido.fecha_entrega = selectedFechaEntrega
          ? formatFecha(selectedFechaEntrega)
          : null;
        pedido.fecha = pedido.fecha ? formatFecha(pedido.fecha) : null;
        try {
          const result = await actualizarPedido(pedido);
  ...
  const confirmDelete = () => {
    ...
    const pedidoAEliminar = pedidos.find(
      (pedido) => pedido.id === selectedPedido
    );
  ...
  const filteredPedidos = pedidos.filter(
    (pedido) =>
      ...
      pedido.nombre_cliente.toLowerCase().includes(searchText.toLowerCase())
  );

  const filteredPedidosByEstadoPedido =
    filterEstadoPedido.length > 0
      ? filteredPedidos.filter((pedido) =>
          filterEstadoPedido.includes(pedido.estado_pedido)
        )
      : filteredPedidos;

  const filteredPedidosByEstadoPago =
    filterEstadoPago.length > 0
      ? filteredPedidosByEstadoPedido.filter((pedido) =>
          filterEstadoPago.includes(pedido.estado_pago)
        )
      : filteredPedidosByEstadoPedido;

  const filteredPedidosByLocalidad =
    filterLocalidad.length > 0
      ? filteredPedidosByEstadoPago.filter(
          (pedido) =>
            filterLocalidad.some(
              (localidad) => localidad.value === pedido.localidad
            ) // Filtrar usando objetos completos
        )
      : filteredPedidosByEstadoPago;

  const filteredPedidosByNombreCliente =
    filterNombreClienteValue.length > 0
      ? filteredPedidosByLocalidad.filter((pedido) =>
          filterNombreClienteValue.includes(pedido.nombre_cliente)
        )
      : filteredPedidosByLocalidad;

  const filteredPedidosByNombreArticulo =
    filterNombreArticuloValue.length > 0
      ? filteredPedidosByNombreCliente.filter((pedido) =>
          filterNombreArticuloValue.includes(pedido.nombre_articulo)
        )
  ...
  const columns = [
      ...
      renderHeader: () => (
        <Checkbox
          checked={
            ...
            filteredPedidosByNombreArticulo.length
          }
          indeterminate={
            selectedVisibleRows.length > 0 &&
            selectedVisibleRows.length < filteredPedidosByNombreArticulo.length
          }
    ...
    { field: "nombre_articulo", headerName: "Artículo", flex: 1.5 },
    {
      ...
      cellClassName: (params) => {
        return params.value !== 1 ? "valorDiferente" : "";
      },
    },
  ...
  const handleSelectAll = (event) => {
    ...
    if (isChecked) {
      ...
      setSelectedVisibleRows(
        filteredPedidosByNombreArticulo.map((pedido) => pedido.id)
      );
      setSelectedRows(
        filteredPedidosByNombreArticulo.map((pedido) => pedido.id)
      );
  ...
  const handleSelect = (id) => {
    ...
    if (isRowSelected) {
      setSelectedVisibleRows((prevSelectedRows) =>
          ...
          <Card className={classes.card}>
            ...
            >
              ...
              <Button
                ...
                startIcon={<EditIcon />}
                className={classes.button}
                onClick={handleUpdateEstadoPedido}
                size="small"
              >
                Estado Pedido
              </Button>
              <Button
                ...
                startIcon={<EditIcon />}
                className={classes.button}
                onClick={handleUpdateEstadoPago}
                size="small"
              >
                Estado Pago
              </Button>
            ...
            <DialogContent>
              {dialogAction === "modificarLocalidad" && (
                <Autocomplete
                  ...
                  onChange={(event, newValue) =>
                    setFilterLocalidadDialog(newValue.value)
                  }
                  renderInput={(params) => (
              ...
              {dialogAction === "modificarEstadoPedido" && (
                <Autocomplete
                  ...
                  onChange={(event, newValue) =>
                    setFilterEstadoPedidoDialog(newValue)
                  }
                  renderInput={(params) => (
                    <TextField {...params} variant="outlined" />
                  )}
              ...
              {dialogAction === "modificarEstadoPago" && (
                <Autocomplete
                  ...
                  onChange={(event, newValue) =>
                    setFilterEstadoPagoDialog(newValue)
                  }
                  renderInput={(params) => (
                    <TextField {...params} variant="outlined" />
                  )}
